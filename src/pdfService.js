...
// Generate Quote PDF
export const generateQuotePDF = async (quote, companySettings = {}, clientData = null, logoInfo = null) => {
  try {
    const doc = new jsPDF();
...
export const generateStatementPDF = async (client, invoices, companySettings = {}, period = 'full', logoInfo = null) => {
  try {
    const doc = new jsPDF();
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(0, 0, 0);
    doc.text(client.name, 20, currentY);
    
    if (client.email) {
      currentY += 7;
      doc.text(client.email, 20, currentY);
    }
    
    if (client.address) {
      currentY += 7;
      doc.text(client.address, 20, currentY);
    }
...
export const generatePDFWithLogo = async (type, data, companySettings, clientData = null, period = null) => {
  try {
    ...
    switch (type) {
      ...
      case 'statement':
        return await generateStatementPDF(data.client, data.invoices, companySettings, period, logoInfo);
      default:
